...
__all__ = ['resnet20_cifar_extend', 'resnet32_cifar_extend', 'resnet44_cifar_extend', 'resnet56_cifar_extend']
...
class ResNetCifar_extend(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=NUM_CLASSES):
        ...
        self.inplanes = 16
        super(ResNetCifar_extend, self).__init__()
    ...
    def forward(self, x):
        ...
        #exit_enable = self.detector(exit_feat)
        ...
        # Add layers
        #print(y.shape,outputx.shape) #batch_size, channel, height, width
        #xnew = outputx + y #sum
        xnew = torch.cat((outputx, y),1) #concat
        #add hard class correction
        exit1 = self.conv1_exit1(xnew)
...
def resnet20_cifar_extend(**kwargs):
    model = ResNetCifar_extend(BasicBlock, [3, 3, 3], **kwargs)
    return model

def resnet32_cifar_extend(**kwargs):
    model = ResNetCifar_extend(BasicBlock, [5, 5, 5], **kwargs)
    return model

def resnet44_cifar_extend(**kwargs):
    model = ResNetCifar_extend(BasicBlock, [7, 7, 7], **kwargs)
    return model

def resnet56_cifar_extend(**kwargs):
    model = ResNetCifar_extend(BasicBlock, [9, 9, 9], **kwargs)
    return model
