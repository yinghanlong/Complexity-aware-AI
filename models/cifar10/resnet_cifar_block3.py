...
__all__ = ['resnet20_cifar_bloc3', 'resnet32_cifar_bloc3', 'resnet44_cifar_bloc3', 'resnet56_cifar_bloc3']
...
class ResNetCifar_bloc3(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=NUM_CLASSES):
        ...
        self.inplanes = 16
        super(ResNetCifar_bloc3, self).__init__()
        ...
        # Define early exit layers
        #self.linear_embed1 = nn.Linear(800,256)
        self.linear_exit1 = nn.Linear(800, num_classes)#nn.Linear(800, num_classes)
        #self.binary_classifier = nn.Linear(800, 2)
        self.layer3 = self._make_layer(self.layer_gates[2], block, 64, layers[2], stride=2)
        self.avgpool = nn.AvgPool2d(8, stride=1)#nn.AvgPool2d(7, stride=1)
        self.fc = nn.Linear(64,  num_classes)#Note: only use the num of hard classes at the third block
        '''
    ...
    def forward(self, x):
        ...
        #exit_feat = self.HCD(x.clone().detach())
        #exit_feat = exit_feat.view(exit_feat.size(0), -1)
        #exit_enable = self.binary_classifier(exit_feat)
        exit1 = nn.functional.avg_pool2d(x, 3)
        exit1 = exit1.view(exit1.size(0), -1)
        #exit1 = self.linear_embed1(exit1)
        #binary classifier to decide whether to exit here or send to the cloud
        #exit_enable = self.binary_classifier(exit1)#exit1.clone().detach())
        exit1 = self.linear_exit1(exit1)
        
        #x = x + y #sum
        x = torch.cat((x, y),1) #concat
        ...
        output.append(x)
        exit_enable=None
        return output, exit_enable


def resnet20_cifar_bloc3(**kwargs):
    model = ResNetCifar_bloc3(BasicBlock, [3, 3, 3], **kwargs)
    return model

def resnet32_cifar_bloc3(**kwargs):
    model = ResNetCifar_bloc3(BasicBlock, [5, 5, 5], **kwargs)
    return model

def resnet44_cifar_bloc3(**kwargs):
    model = ResNetCifar_bloc3(BasicBlock, [7, 7, 7], **kwargs)
    return model

def resnet56_cifar_bloc3(**kwargs):
    model = ResNetCifar_bloc3(BasicBlock, [9, 9, 9], **kwargs)
    return model
