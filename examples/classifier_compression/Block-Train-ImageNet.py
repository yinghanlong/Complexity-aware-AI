...
from models.imagenet.resnet_block2 import BasicBlock
# Logger handle
msglogger = None
...
def adjust_learning_rate(optimizer, epoch):
    #update_list = [180,220]
    update_list = [30,60,85]#[60,120,160]#[230,290,330]#[60,120,160]#[200,250,300]
...
def main():
    ...
    model.module.conv1_exit1 = nn.Conv2d(128, 12, kernel_size=7, stride=2, padding=3, bias=True)
    model.module.fc_exit1 = nn.Linear(768, 1000)
    ...
    model = create_model(args.pretrained, args.dataset, 'resnet18_b2',parallel=not args.load_serialized, device_ids=args.gpus)
    # Define early exit layers
    model.module.conv1_exit1 = nn.Conv2d(128, 12, kernel_size=7, stride=2, padding=3, bias=True)
    model.module.fc_exit1 = nn.Linear(768, 1000)
    model.to(args.device)
    ...
    model.module.fc = torch.nn.Linear(512,args.num_hard).to(args.device) #reduce the number of classes to only include hard classes
    optimizer = torch.optim.SGD(model.parameters(), lr=args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)
