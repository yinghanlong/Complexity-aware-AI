...
def main():
    ...
    #blocktrain-cifar100-70/hard_classes.pickle
    with open('blocktrain_resnet32_50/hard_classes.pickle','rb') as f:
        args.hard_dict = {}
    ...
    if args.evaluate:
        #hard/easy class detector    
        #model.module.HCD= nn.Sequential(
        #    BasicBlock([True,True], 32, 32),
        #    nn.AvgPool2d(3)
        #).to(args.device)        
        model.module.fc = torch.nn.Linear(64,args.num_hard).to(args.device) #reduce the number of classes to only include hard classes
        ...
        model.module.corrector = nn.Sequential(
            ...
            BasicBlock([True,True], 16, 32, stride=2, downsample=downsample)
            #model.module._make_layer([[True,True]], BasicBlock, 16, 1),
            #model.module._make_layer([[True,True]], BasicBlock, 32, 1, stride=2)
        ).to(args.device)

        #TODO:if using concatenation, redefine extension block
        if args.concat==True:
            ...
            model.module.layer3= nn.Sequential(
	            BasicBlock([True,True],64,64, stride=2, downsample=downsample1),
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):   
        ...
        apputils.save_checkpoint(epoch, args.arch, model, optimizer, compression_scheduler,
                                 best_epochs[-1].top1, is_best,'block1', msglogger.logdir)
    
    # Finally run results on the test set and select hard classes
    ...
    args.blockidx = 1
    '''
    #model = create_model(args.pretrained, args.dataset, 'resnet32_cifar_bloc2',parallel=not args.load_serialized, device_ids=args.gpus)
    #model.module.fc = torch.nn.Linear(256,args.num_hard).to(args.device) #reduce the number of classes to only include hard classes
    #model, _,_ = apputils.load_checkpoint(model, chkpt_file=args.resume)#os.path.join(msglogger.logdir,'block1_checkpoint.pth.tar'))
    #model.to(args.device)
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):   
        ...
        for score in reversed(best_epochs):
            if score.top1 > 0:
                msglogger.info('==> Best Top1: %.3f on Epoch: %d', score.top1, score.epoch)
        apputils.save_checkpoint(epoch, args.arch, model, optimizer, compression_scheduler,
                                 best_epochs[-1].top1, is_best, 'block2', msglogger.logdir)
    
    
    #run results on the test set
    ...
    #model = create_model(args.pretrained, args.dataset, 'resnet32_cifar_bloc3',parallel=not args.load_serialized, device_ids=args.gpus)
    ...
    #    nn.AvgPool2d(3)
    #).to(args.device)
    #model, _,_ = apputils.load_checkpoint(model, chkpt_file=os.path.join(msglogger.logdir,'block2_checkpoint.pth.tar'))
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):   
        ...
        for child in model.modules():
            ...
            '''
            if ct>2 and ct<=81: #layer 81: early exit
            #if ct>2 and ct<=83 or ct>=125 and ct<=133:#ct<=55:#ct<=83:# Layer 83: hard class detector# TODO:the num of layers to freeze depends
                #print("Fix Layer ",ct,child)
    ...
    for train_step, (inputs, target) in enumerate(train_loader):
        ...
        if args.blockidx>1:
            ...
            for b in range(inputs.size(0)):
                ...
                pred_labels_b= edge_pred.item() in args.hard_dict
...
def _validate(data_loader, model, criterion, loggers, args, bin_op,epoch=-1):
    ...
    for validation_step, (inputs, target) in enumerate(data_loader):
        with torch.no_grad():
            ...
            if args.blockidx>1:#Test the Hard class detector at second block
                #binary classifier loss
                target_batch = []
                for b in range(inputs.size(0)):
                    #target_batch.append(torch.equal(edge_pred,target[b]))
                ...
                #Detect hard/easy classes
                for b in range(inputs.size(0)):
                    out_max = F.softmax(output[0][b])
                    #_, top2_pred = torch.topk(out_max,2)
                    edge_confid, edge_pred = torch.topk(out_max,1)
                    #second_is_easy = top2_pred[1].item() not in args.hard_dict
                    is_easy = (edge_pred.item() in args.hard_dict)==False# and edge_confid>0.2
                    pred_labels_b= not is_easy#pred_labels[b]# or ~((edge_pred.item() in args.hard_dict)==False and edge_confid>0.3)#TODO
                    if bool(pred_labels_b) == bool(target_batch[b]):
                        correct_sp+=1
    ...
    if args.blockidx==1:#decide easy/hard classes
        ...
        #        new_label+=1
        '''
        _, args.hard_class_idx =torch.topk(-1.0 *args.class_accuracy,args.num_hard) #take negative to reverse ordering
        args.hard_dict={}
        new_label = 0
...
def earlyexit_validate_loss(output, targets, pred_labels, criterion, args):
    ...
    for batch_index in range(this_batch_size):
        ...
        out_max = F.softmax(output[1][batch_index])      
        ...
        if args.blockidx>=2:
          for exitnum in range(0,len(output) - 1):
            ...
            if exitnum ==0:
                is_easy = (edge_pred.item() in args.hard_dict)==False# and edge_confid>0.3# and (args.loss_exits[exitnum][batch_index] < args.earlyexit_thresholds[exitnum])#  or (edge_confid> confidence)#(hard_prob< args.num_hard/args.num_classes) #(edge_pred.item() in args.hard_dict)==False and edge_confid>0.3
                ...
                is_easy = False#(args.loss_exits[exitnum][batch_index] < args.earlyexit_thresholds[exitnum])# and edge_confid>0.3
            ...
            if args.cloud==True:
                if (args.loss_exits[0][batch_index] > args.earlyexit_thresholds[0]):
                    break
            #if predicted as easy or predicted as hard but confidence at the main block is higher
            if exitnum==0:#(exitnum==0 and is_easy ) or exitnum==1:
...
def evaluate_model(model, criterion, test_loader, loggers, activations_collectors, args,bin_op, scheduler=None):
    ...
    '''
    print('easy and correct confidence',np.std(ec_confid),np.mean(ec_confid))
    print('easy and wrong confidence',np.std(ew_confid),np.mean(ew_confid))
    print('hard and correct confidence',np.std(hc_confid),np.mean(hc_confid))
    print('hard and wrong confidence',np.std(hw_confid),np.mean(hw_confid))
    print('easy and correct entropy',np.std(ec_entro),np.mean(ec_entro))
    print('easy and wrong entropy',np.std(ew_entro),np.mean(ew_entro))
    print('hard and correct entropy',np.std(hc_entro),np.mean(hc_entro))
    print('hard and wrong entropy',np.std(hw_entro),np.mean(hw_entro))
    #plt.hist(ec_confid,bins=100)
    #plt.show()
    #plt.savefig('ec_confid.pdf')
    #plt.hist(ew_confid,bins=100)
    #plt.show()
    #plt.savefig('ew_confid.pdf')
    #plt.hist(hc_confid,bins=100)
    #plt.show()
    #plt.savefig('hc_confid.pdf')
    #plt.hist(hw_confid,bins=100)
    #plt.show()
    #plt.savefig('hw_confid.pdf')
    plt.hist(ec_entro,bins=100)
    plt.show()
    #plt.savefig('ec_entro.pdf')
    plt.hist(ew_entro,bins=100)
    plt.show()
    #plt.savefig('ew_entro.pdf')
    plt.hist(hc_entro,bins=100)
    plt.show()
    #plt.savefig('hc_entro.pdf')
    plt.hist(hw_entro,bins=100)
    plt.show()
    #plt.savefig('hw_entro.pdf')
    
    ec_confid.extend(ew_confid)
    hc_confid.extend(hw_confid)
    ec_entro.extend(ew_entro)
    hc_entro.extend(hw_entro)
    print('easy confidence',np.std(ec_confid),np.mean(ec_confid))
    print('hard confidence',np.std(hc_confid),np.mean(hc_confid))
    print('easy entropy',np.std(ec_entro),np.mean(ec_entro))
    print('hard entropy',np.std(hc_entro),np.mean(hc_entro))
    '''
