...
from models.imagenet.mobilenetv2 import InvertedResidual as block#LinearBottleNeck
from models.imagenet.mobilenetv2 import ConvBNReLU
...
def adjust_learning_rate(optimizer, epoch):
    #update_list = [180,220]
    update_list = [30,60,110]#[60,120,160]#[230,290,330]#[60,120,160]#[200,250,300]
...
def main():
    ...
    with open('mobilenet_imagenet/hard_classes.pickle','rb') as f:
        args.hard_dict = {}
    ...
    #model.module.load_state_dict(torch.load('mobilenet_v2-b0353104.pth'),strict=False)
    #add extension
    ...
    model.module.corrector = nn.Sequential(*features)

    model.module.extension =nn.Sequential(
        block(640, 320, stride, expand_ratio=1),#process concatenated inputs
        block(320, 320, stride, expand_ratio=3),
        ConvBNReLU(input_channel, last_channel, kernel_size=1)
        )
    model.module.classifier2 = nn.Linear(1280, args.num_hard)
    '''
    model.module.corrector = nn.Sequential(
        ...
        model.module._make_stage(1, 16, 24, 2, 6),
        model.module._make_stage(1, 24, 32, 2, 6),
        model.module._make_stage(1, 32, 64, 2, 6),
        model.module._make_stage(1, 64, 96, 1, 6),
        model.module._make_stage(1, 96, 160, 1, 6),
        model.module._make_stage(1, 160, 320, 1, 6)
    )
    model.module.extension = model.module._make_stage(2, 320, 320, 1, 6)

    model.module.conv3 = nn.Sequential(
   ...
   #-----------------------------#
    ...
    start_epoch=0
    args.blockidx = 2
    #model.module.conv4 = nn.Conv2d(1280, 50, 1).to(args.device)
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):   
        ...
        for child in model.modules():
            ct+=1
            #Fix layers in the main block
            '''
            if ct>2 and ct<=214: #and ct<=186:#ct<=55:#ct<=83:# Layer 83: hard class detector# TODO:the num of layers to freeze depends on the num of binary layers
        ...
        #trained_params_num = sum([np.prod(p.size()) for p in trained_params])
        #print(params, trained_params_num)
        ...
        with torch.no_grad():
            ...
            if args.blockidx>=2:#Test the Hard class detector at second block
                ...
                for b in range(inputs.size(0)):
                    out_max = F.softmax(output[0][b])
                    edge_confid, edge_pred = torch.topk(out_max,1)
                    pred_labels_b= (edge_pred.item() in args.hard_dict)#~((edge_pred.item() in args.hard_dict)==False and edge_confid>0.3)#TODO (edge_pred.item() in args.hard_dict)
                    if bool(pred_labels_b) == bool(target_batch[b]):
                        correct_sp+=1
