...
from models.cifar10.resnet_cifar_block2 import BasicBlock
import matplotlib.pyplot as plt
...
error_type = np.zeros(4)
correct_pt_x=[]#class-wise complexity
correct_pt_y=[]#instance-wise complexity
wrong_pt_x=[]#class-wise complexity
wrong_pt_y=[]#instance-wise complexity
...
def adjust_learning_rate(optimizer, epoch):
    #update_list = [180,220]
    update_list = [60,120,160]#[230,290,330]#[60,120,160]#[200,250,300]
...
def main():
    ...
    args.num_classes = 100 if args.dataset == 'cifar100' else 1000
    args.num_hard = 50
    ...
    with open('resnet32-hardclass/hard_classes.pickle','rb') as f:
        args.hard_dict = {}
        '''
        for i in range(50):#TODO:random class
            args.hard_dict[i]= i
        '''
    ...
    if args.evaluate or args.resume:        
        ...
        downsample0 = nn.Sequential(
                nn.Conv2d(128, 64,
                          kernel_size=1,bias=False),#stride=2
                nn.BatchNorm2d(64),
        )
        if args.concat==True:
            model.module.conv1_exit1 = nn.Sequential(
                BasicBlock([True,True],128,64,  downsample=downsample0),#concat two inputs, use a wider conv layer
                BasicBlock([True,True],64,64),
                BasicBlock([True,True], 64,64),
                BasicBlock([True,True],64,64)
            )
            #model.module.avgpool=nn.AdaptiveAvgPool2d((1,1))#nn.AvgPool2d(8, stride=1)
            model.module.fc_exit1 = nn.Linear(64, args.num_hard)#reduce the number of classes to only include hard classes
        ...
        model.module.corrector = nn.Sequential(
            ...
            BasicBlock([True,True], 16,16),
            BasicBlock([True,True], 16, 32, stride=2, downsample=downsample1),
            #BasicBlock([True,True],32,32),#add more layers
            BasicBlock([True,True], 32, 64, stride=2, downsample=downsample2)#,
            #BasicBlock([True,True],64,64)#add more layers
        )
    ...
    if args.evaluate:
        ...
        model_cloud = create_model(args.pretrained, args.dataset, 'resnet32_cifar',
                         parallel=not args.load_serialized, device_ids=args.gpus)
        macs,params = get_model_complexity_info(model_cloud,(3,32,32),as_strings=True)
        print(macs,params)
        #return 1
    ...
    if args.concat==True:
        ...
        model.module.conv1_exit1 = nn.Sequential(
	        BasicBlock([True,True],128,64, downsample=downsample0),#concat two inputs, use a wider conv layer #stride=2
            BasicBlock([True,True], 64, 64),
            BasicBlock([True,True],  64, 64),
            BasicBlock([True,True],  64, 64)
            )
        ...
        #model.module.avgpool=nn.AdaptiveAvgPool2d((1,1))#nn.AvgPool2d(8, stride=1)
        model.module.fc_exit1 = nn.Linear(64, args.num_hard)#nn.Linear(128, args.num_hard)
    else:
        model.module.conv1_exit1 = nn.Sequential(
	    BasicBlock([True,True], 64, 64),
        BasicBlock([True,True], 64, 64),
    ...
    model.module.corrector = nn.Sequential(
        ...
        BasicBlock([True,True], 16,16),
        BasicBlock([True,True], 16, 32, stride=2, downsample=downsample1),
        #BasicBlock([True,True],32,32),
        BasicBlock([True,True], 32, 64, stride=2, downsample=downsample2)#,
        #BasicBlock([True,True],64,64)#add more layers
    )
    
    #define easy/hard class detector
    #model.module.HCD= nn.Sequential(
    #    BasicBlock([True,True],64,64),
    #    nn.AvgPool2d(8, stride=1)
    #)
    #model.module.detector= nn.Linear(64,  2)
    model.to(args.device)
    optimizer = torch.optim.SGD(model.parameters(), lr=args.lr,
                                momentum=args.momentum,
   ...
   #-----------------------------#
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):   
        ...
        for child in model.modules():
            ...
            if ct>2 and ct<=121:#ct<=55:#ct<=83:# Layer 83: hard class detector# TODO:the num of layers to freeze depends on the num of layers
...
def train(train_loader, model, criterion, optimizer, epoch,
    ...
    for train_step, (inputs, target) in enumerate(train_loader):
        ...
        if args.blockidx <2:
            ...
            for b in range(inputs.size(0)):
                if target[b].item() in args.hard_dict:
                    ...
                    target_hard[b] = torch.tensor(0)
            #select only samples of hard classes for training
        ...
        output = model(inputs)
        #predict if belonging to easy/hard classes using a detector
        #output, exit_enable = model(inputs)
        ...
        if args.blockidx>=2:
            ...
            for b in range(inputs.size(0)):
                ...
                if targets[0][b].item() in args.hard_dict:
            ...
            for b in range(inputs.size(0)):
                ...
                #pred_labels_b= pred_labels[b]
                pred_labels_b= edge_pred.item() in args.hard_dict#~((edge_pred.item() in args.hard_dict)==False and edge_confid>0.3)
        ...
        if not args.earlyexit_lossweights:
            ...
            #TODO: Measure accuracy and record loss of all exits
        ...
        if args.blockidx==1:
            ...
            #TODO:train the extension and adaptive blocks 
            loss.backward()
            #TODO:train the easy/hard class detector
            #loss_exit.backward()
...
def _validate(data_loader, model, criterion, loggers, args, bin_op,epoch=-1):
    ...
    #enable cloud network or not
    if args.cloud == True:
        model_cloud = create_model(args.pretrained, args.dataset, 'resnet101_cifar',
                         parallel=not args.load_serialized, device_ids=args.gpus)
        model_cloud, _,_ = apputils.load_checkpoint(model_cloud, chkpt_file='resnet101_cifar100_lr0.2/fullprecision_best.pth.tar')
        model_cloud.eval()
    ...
    for validation_step, (inputs, target) in enumerate(data_loader):
        with torch.no_grad():
            ...
            output= model(inputs)
            #TODO:predict easy/hard classes using a detector
            #output, exit_enable= model(inputs)
            # measure elapsed time
            batch_time.add(time.time() - end)     
            ...
            if args.blockidx>=2:#Test the Hard class detector at second block
                ...
                for b in range(inputs.size(0)):
                    #target_batch.append(torch.equal(edge_pred,target[b]))
                    if target[b].item() in args.hard_dict:
                        target_batch.append(True)#args.class_complex[target[b]]) #easy class = True, hard class = False
                    else:
                        target_batch.append(False)
                target_batch=torch.LongTensor(target_batch).cuda()
                #loss_exit =  criterion(exit_enable, target_batch)
                pred_labels = target_batch #force it to be correct
                ...
                for b in range(inputs.size(0)):
                    out_max = F.softmax(output[0][b])
                    _, top2_pred = torch.topk(out_max,2)
                    edge_confid, edge_pred = torch.topk(out_max,1)
                    second_is_easy = top2_pred[1].item() not in args.hard_dict
                    is_easy = (edge_pred.item() in args.hard_dict)==False #and (edge_confid>confid_thre or second_is_easy)
                    pred_labels_b= not is_easy
                    #pred_labels_b= pred_labels[b]
                    if bool(pred_labels_b) == bool(target_batch[b]):
                        correct_sp+=1
    ...
    #record class level complexity for each data point 
    '''                                           
    for pt in range(len(correct_pt_x)):
        pt_class=correct_pt_x[pt]
        correct_pt_x[pt] = 1.0 - args.class_accuracy[pt_class]                                    
    for pt in range(len(wrong_pt_x)):
        pt_class=wrong_pt_x[pt]
        wrong_pt_x[pt] = 1.0 -args.class_accuracy[pt_class]
    '''                                
...
def earlyexit_validate_loss(output, targets, pred_labels, criterion, args):
    ...
    for batch_index in range(this_batch_size):
        ...
        confidence, _ = torch.topk(out_max,1)#confidence at the last exit of edge
        if args.blockidx>=2: #allow early exiting
          for exitnum in range(0,len(output) - 1):
            ...
            if exitnum ==0:
                is_hard = (edge_pred.item() in args.hard_dict)
                is_easy = (edge_pred.item() not in args.hard_dict)# and edge_confid>0.3# and (edge_confid>confid_thre or second_is_easy)#(hard_prob< args.num_hard/args.num_classes) #(edge_pred.item() in args.hard_dict)==False and edge_confid>0.3
                truth_is_easy = (targets[exitnum][batch_index].item() in args.hard_dict)==False
                _, top5_pred = torch.topk(out_max,5)
                top5_hard = False
                for i in range(5):
                    if top5_pred[i].item() in args.hard_dict:
                        top5_hard =True
                #second_is_easy = top2_pred[1].item() not in args.hard_dict
                likely_hard = top5_hard and (args.loss_exits[exitnum][batch_index] > args.earlyexit_thresholds[exitnum])#type 2 error
                #is_easy =(bool(pred_labels[batch_index])==False)
                #entropy distribution
                
                if is_easy:
                    if torch.equal(edge_pred, targets[exitnum][batch_index]): #correct
                        ec_confid.append(edge_confid)#easy and correct
                        ec_entro.append(args.loss_exits[exitnum][batch_index])
                    else:
                        ew_confid.append(edge_confid)
                        ew_entro.append(args.loss_exits[exitnum][batch_index])
                else:#hard
                    if torch.equal(edge_pred, targets[exitnum][batch_index]): #correct
                        hc_confid.append(edge_confid)
                        hc_entro.append(args.loss_exits[exitnum][batch_index])
                    else:
                        hw_confid.append(edge_confid)
                        hw_entro.append(args.loss_exits[exitnum][batch_index])
                '''
                if is_hard and (args.loss_exits[exitnum][batch_index] >0.5) and (args.loss_exits[exitnum+1][batch_index] <0.05):
                    is_easy=True  
                          
                    #select first easy class
                    force_label = top5_pred[1].item()
                    for i in range(5):
                        if top5_pred[i].item() not in args.hard_dict:
                            force_label=top5_pred[i].item()
                            break
                    output[exitnum][batch_index][force_label]= output[exitnum][batch_index][edge_pred.item()]
                    output[exitnum][batch_index][edge_pred.item()]= 0
                '''
            ...
            #if (args.evaluate and exitnum==0 and (is_easy or edge_confid>confidence)) or exitnum==1:
            #if is_hard or (is_easy and (not likely_hard or args.loss_exits[exitnum][batch_index] < args.loss_exits[exitnum+1][batch_index])):#(edge_confid> confidence):# or (args.loss_exits[exitnum][batch_index] < args.earlyexit_thresholds[exitnum]):

                if torch.equal(edge_pred, targets[exitnum][batch_index]): #correct
                    args.class_correct[edge_pred]+=1
                    #correct_pt_y.append(edge_confid)
                    #correct_pt_x.append(edge_pred.item())#class
                #else:
                #    wrong_pt_y.append(edge_confid)
                #    wrong_pt_x.append(targets[exitnum][batch_index].item())#class
                args.class_cnt[edge_pred]+=1 #precision
                '''
                #count the number of different types of mispredictions at the main block
                if torch.equal(edge_pred, targets[exitnum][batch_index])==False: #mispredict
                    if truth_is_easy and not is_easy:#easy predicted as hard
                        error_type[0]+=1
                    elif not truth_is_easy and is_easy:#hard predicted as easy
                        error_type[1]+=1
                    elif truth_is_easy and is_easy:#easy predicted as another easy class
                        error_type[2]+=1
                    else:#hard predicted as another hard class
                        error_type[3]+=1
                '''
        ...
        if not earlyexit_taken:
            ...
            '''
            #count the number of different types of mispredictions at the extension block
            truth_is_easy = (targets[0][batch_index].item() in args.hard_dict)==False
            if torch.equal(edge_pred, targets[exitnum][batch_index])==False: #mispredict
                if truth_is_easy:#easy predicted as hard
                    error_type[0]+=1
                else:#hard predicted as another hard class
                    error_type[3]+=1
            '''
            ...
            if torch.equal(edge_pred, targets[exitnum][batch_index]): #correct
                extend_hc_confid.append(confidence)
                extend_hc_entro.append(args.loss_exits[exitnum][batch_index])
            else:
                if truth_is_easy:
                    extend_hw1_confid.append(confidence)
                    extend_hw1_entro.append(args.loss_exits[exitnum][batch_index])
                else:
                    extend_hw4_confid.append(confidence)
                    extend_hw4_entro.append(args.loss_exits[exitnum][batch_index])
        #plot entropy distribution
        #if truth_is_easy and not is_easy:
            #prediction is wrong, type 1
            #extend1_entro.append(args.loss_exits[exitnum][batch_index])
        #elif is_easy and not truth_is_easy:#type 2
            #extend2_entro.append(args.loss_exits[exitnum][batch_index])
...
def evaluate_model(model, criterion, test_loader, loggers, activations_collectors, args,bin_op, scheduler=None):
    ...
    #print(error_type)
    ew_entro.extend(hw_entro)
    ec_entro.extend(hc_entro)
    print('wrong entropy',np.std(ew_entro),np.mean(ew_entro))
    print('correct entropy',np.std(ec_entro),np.mean(ec_entro))
    #print('hard and wrong entropy',np.std(hw_entro),np.mean(hw_entro))
    '''
    fig, axs = plt.subplots()
    axs.plot(correct_pt_x,correct_pt_y,'g+')
    axs.plot(wrong_pt_x,wrong_pt_y,'r+')
    axs.set(xlabel='class-wise complexity(loss)',ylabel='instance-wise complexity(entropy)')
    fig.savefig('complexity.pdf')
    plt.show()
    
    fig, axs = plt.subplots(1,2, sharey=True, tight_layout=True)
    axs[0].hist(ec_entro,bins=100)
    axs[1].hist(ew_entro,bins=100)
    plt.savefig('resnet32_entro.pdf')
    plt.show()    
    
    fig2, axs2 = plt.subplots(2,2, tight_layout=True)
    axs2[0][0].hist(extend_hc_entro,bins=100)
    axs2[0][1].hist(extend_hw1_entro,bins=100)
    axs2[1][1].hist(extend_hw4_entro,bins=100)
    plt.savefig('extend_entro.pdf')
    plt.show()
    
    fig, axs = plt.subplots(2,2, tight_layout=True)
    axs[0][0].hist(ec_confid,bins=100)
    axs[0][1].hist(ew_confid,bins=100)
    axs[1][0].hist(hc_confid,bins=100)
    axs[1][1].hist(hw_confid,bins=100)
    plt.savefig('main_confid.pdf')
    plt.show()    
    fig2, axs2 = plt.subplots(2,2, tight_layout=True)
    axs2[0][0].hist(extend_hc_confid,bins=100)
    axs2[0][1].hist(extend_hw1_confid,bins=100)
    axs2[1][1].hist(extend_hw4_confid,bins=100)
    plt.savefig('extend_confid.pdf')
    plt.show()
    '''
