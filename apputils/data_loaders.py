...
def load_data(dataset, data_dir, batch_size, workers, valid_size=0.1, deterministic=False):
    ...
    if dataset == 'cifar100':
        return cifar100_load_data(data_dir, batch_size, workers, valid_size=valid_size, deterministic=deterministic)
    if dataset == 'imagenet':
...
def cifar100_load_data(data_dir, batch_size, num_workers, valid_size=0.1, deterministic=False):
    """Load the CIFAR10 dataset.
